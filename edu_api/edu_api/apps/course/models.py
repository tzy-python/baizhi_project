...
class Course(BaseModel):
    ...
    @property
    def brief_html(self):
        # src="/media/2020/07/14/u6543529444097945658fm26gp0.jpg"
...
class CourseLesson(BaseModel):
    ...
    @property
    def lesson_course(self):
        lesson_courses = CourseLesson.objects.filter(is_show=True, is_delete=False, course_id=self.id).all()

        data_list = []
        for lesson in lesson_courses:
            data_list.append({
                "id": lesson.id,
                "name": lesson.name,
                "free_trail": lesson.free_trail,
